...
const proxyModule = buildModule("ProxyModule", (m: any) => {
  ...
  const livenessRadius = m.contract("LivenessRadius");

  const encodedFunctionCall = m.encodeFunctionCall(livenessRadius, "initialize");
  ...
  const proxy = m.contract("TransparentUpgradeableProxy", [livenessRadius, proxyAdminOwner, encodedFunctionCall]);
 ...
 */
const livenessRadiusModule = buildModule("LivenessRadiusModule", (m: any) => {
  ...
  const livenessRadius = m.contractAt("LivenessRadius", proxy);

  // Return the contract instance, along with the original proxy and proxyAdmin contracts
  // so that they can be used by other modules, or in tests and scripts.
  return { livenessRadius, proxy, proxyAdmin };
});

module.exports = livenessRadiusModule;
